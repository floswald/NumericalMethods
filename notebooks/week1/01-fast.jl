...
# first time you open this notebook, Pluto will automatically create an environment and download all the packages. it will take couple of minutes. first time only.
# if you remove a package here, Pluto will alter the Manifest.toml associated to that environment.
begin
...
let's try R now. the `RCall.jl` package is great for this. We just call the built-in `sum` function:
"""
...
md"
R comes in as $(relc(:R,d)) times C, but at $(rely(:R,:python,d)) times the python timing.
"
...
# ╔═╡ 09b12eda-a252-4a89-a2a6-dddefb9d7bd6
md"
We are at $(relc(:julia_hand,d)) times C, not bad at all! 

## Overview!

Let's list all those times relative to C and compare:
"

# ╔═╡ fed8b4f0-55e5-11eb-2cc2-990221ad2789
begin
	df = DataFrame(language = collect(keys(d)), time = collect(values(d)))
	transform!(df, [:language, :time] => ((x,y) -> y ./ y[x .== :C]) => :rel_time)
	sort!(df, :time)
end
