...
This is identical to the Aiyagari notebook but it introduces a minmum consumption level cbar.
"""

# ╔═╡ c1388e32-4a26-11ec-3def-9d7f551edcca
md"""

>this implementation does not work for the problem: either the problem becomes infeasible ($c < \bar{c}$) or the constraint is without any effect. need EGM to get lower bound right.
...
same implementation, just have $\bar{c}$ as the lower bound instead of 0 on consumption.
...
$$R[i,j] = \begin{cases}u( w z(i) + (1 + r)a(i) - a_j) & \text{if }w z(i) + (1 + r)a(i) - a_j > \bar{c} \\
...
function setup_R!(R, a_vals, s_vals, r, w, u, cbar)
...
Household = @with_kw (r = 0.01,
	                cbar = 0.05,  # new parameter!
                      ...
                      R = setup_R!(fill(-Inf, n, a_size), a_vals, s_vals, r, w, u, cbar),
...
# ╔═╡ 450c428c-77ac-4c16-a054-d247aea255c5
md"""
``\bar{c}``: $(@bind cbar PlutoUI.Slider(0.0:0.01:10; show_value=true, default=0.0))
"""
...
begin
	...
	am = Household(a_max = 20.0, r = 0.03, w = 0.956, z_chain = MarkovChain([0.9 0.1; 0.1 0.9], [10.0; 20.0]) , cbar = cbar)
...
let
	...
	plot(plotc(am),
		plot(am.a_vals,vstar, legend = :bottomright, label = ["low z" "high z"],
	     xlab = "a",ylab = "V", title = "Value function"),
		layout = (1,2))
