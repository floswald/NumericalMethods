...
Household = @with_kw (r = 0.01,
	...
	ϵ = 0.4,
	y_size = 5,
	yvec = sqrt(2.0) * ϵ .* gausshermite(y_size)[1],
	ywgt = gausshermite(y_size)[2] .* π^(-0.5),
	a_min = 1e-10,
	a_max = 5.0,
	a_size = 200,
	...
	up = σ == 1 ?  x -> 1 / x :  x -> x^(-σ),
	iup = σ == 1 ?  x -> 1 / x :  x -> x^(-1/σ),
	m = vcat(0.0, collect(a_vals)),
	c = collect(range(0.0,a_max,length=a_size+1)),
	mnew = zeros(length(m)),
	cnew = zeros(length(m)),
	m1 = exp.(yvec) .+ (1 + r) * a_vals',
	cfloor = 0.0001,
	tol = 1e-6
		
)

# ╔═╡ 344ca865-76e7-4413-b060-ae04958e7fd5
function EGMstep!(hh)
	# interpolate cons function and get future consumption at all states
    citp = LinearInterpolation(hh.m,hh.c, extrapolation_bc=Line())
    c1 = reshape( citp(hh.m1[:]), hh.y_size,hh.a_size )
    
    # consumption constraint
    c1[c1 .<= hh.cbar] .= hh.cfloor

	# Expected marginal utility at each state
    μ = hh.ywgt' * hh.up.(c1)
    ...
    hh.cnew[:] .= vcat(hh.cbar, cnew)
    hh.mnew[:] .= vcat(hh.cbar, mnew)

end

# ╔═╡ de13da75-74d2-4cd0-be0b-6cce5eea4ece
differ(hh) = max(maximum(abs.(hh.m - hh.mnew)), maximum(abs.(hh.c - hh.cnew)))

# ╔═╡ ff3d91df-69ab-4e38-9a42-7dadfdf1526b
function solve!(hh)
    diff = 1e12
    # a.pmod = 1.0
    while diff > hh.tol
...
# ╔═╡ 75552243-143b-4640-81cd-64d4b43b59b9
md"""
``\bar{c}``: $(@bind cb PlutoUI.Slider(0.0:0.01:10; show_value=true, default=0.0))
"""

# ╔═╡ 6e6ccee2-2006-4794-a1b8-16ec70513923
h = Household(cbar = cb);

# ╔═╡ 4a10ada7-60b5-438f-ad90-ae02befb6be6
solve!(h)

# ╔═╡ 9a377699-faee-47a2-9eb5-736781d3946c
let
	plot(h.m,h.c,xlims = (0,Inf),ylims = (0,Inf), leg = false)
	hline!([cb])
end
